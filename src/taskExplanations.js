...
- Точечная подписка через несколько \`useSelector\` (вместо деструктуризации одного объекта) в связке с мемоизированными селекторами — способ минимизировать лишние ререндеры компонента.`,
  ...
  // ==========================================
  r15: `#### � Суть задачи
Комплексный рефакторинг реального бизнес-компонента со всеми типовыми ошибками собеседований (утечка таймера, мутирующая сортировка, отсутствие useCallback).

#### �️ Пошаговые этапы решения:

1. **Мемоизация компонента и коллбэка**:
   Оборачиваем элемент списка в \`memo\` и передаем обработчик через \`useCallback\`.
   \`\`\`tsx
...
2. **Иммутабельная сортировка**:
   Делаем копию массива через \`.slice()\` перед вызовом метода \`.sort()\`.
   \`\`\`ts
...
3. **Очистка интервала**:
   Сохраняем ID таймера в \`useRef\` и обязательно вызываем \`clearInterval\` при размонтировании.

#### � Ключевые выводы:
- Данный комплексный рефакторинг является эталонным образцом проверки квалификации фронтенд-разработчика.`,
...
1.
**Структура классического цикла \`for\`**:
...
2.
**Опущение управляющих выражений (бесконечный цикл)**:
...
1.
**Проверка четности через остаток от деления**:
...
2.
**Альтернативная оптимизация цикла**:
...
1.
**Нормализация регистра и обратный цикл \`for\`**:
...
2.
**Альтернативное решение через методы массивов**:
\`\`\`js
...
1.
**Итерация по массиву через цикл \`for\`**:
...
2.
**Декларативное решение через \`reduce\`**:
\`\`\`js
...
1.
**Итерация по значениям в цикле \`for...of\`**:
\`\`\`js
const sumNumbers = (arr) => {
  let sum = 0;
...
1.
**Линейный поиск с ранним возвратом**:
\`\`\`js
...
2.
**Использование встроенного метода \`includes()\`**:
\`\`\`js
...
1.
**Императивное накопление в результирующий массив**:
\`\`\`js
...
2.
**Декларативное решение через \`filter()\`**:
\`\`\`js
...
1.
**Указание стартового индекса поиска**:
\`\`\`js
const arr = [1, 7, 3, 7, 5];
...
1.
**Использование метода \`find()\` с предикатом**:
\`\`\`js
...
1.
**Проверка длины строки в предикате**:
\`\`\`js
...
1.
**Поиск по условию отрицательности**:
\`\`\`js
...
1.
**Выполнение кода при отсутствии совпадений**:
\`\`\`js
const arr = [1, 3, 5, 7];
...
1.
**Короткая запись \`filter(Boolean)\`**:
\`\`\`js
...
1.
**Фильтрация по свойству объекта**:
\`\`\`js
const users = [
  { name: "John", age: 17 },
  { name: "Jane", age: 20 },
...
1.
**Проверка двух условий с логическим И (\`&&\`)**:
\`\`\`js
...
1.
**Проверка вхождения статуса в список**:
\`\`\`js
...
• Связка \`filter()\` + \`statuses.includes()\` — эффективный паттерн для множественного выбора в веб-приложениях.`,
  ...
  js34: `#### 🎯 Суть задачи
Разбор работы предиката \`num => num\`, возвращающего сам элемент массива.

#### 🛠️ Пошаговые этапы решения:

1.
**Оценка истинности положительных чисел**:
\`\`\`js
const arr = [1, 2, 3];
...
\`\`\`
📌 **Разбор**: Предикат \`num => num\` возвращает само число. Положительные числа \`1, 2, 3\` являются truthy (истинными) значениями, поэтому предикат дает \`true\` для каждого из них и возвращает копию всего массива.

#### 📌 Ключевые выводы:
• Любые ненулевые числа в JavaScript приводятся к \`true\`.
• \`filter()\` в любом случае вернет новый массив.`,
...
1.
**Применение строкового метода в колбэке**:
\`\`\`js
...
• \`map()\` умеет изменять структуру данных и форматировать нескольких полей одновременно.`,
  ...
  js40: `#### 🎯 Суть задачи
Иммутабельное добавление нового свойства в каждый объект массива при помощи оператора spread (\`...\`).

#### 🛠️ Пошаговые этапы решения:

1.
**Фиксированное добавление свойства**:
\`\`\`js
const products = [
...
\`\`\`
📌 **Разбор**: Оператор \`{ ...product, inStock: true }\` скопирует все существующие свойства продукта в новый объект и добавит свойство \`inStock: true\`.

2.
**Универсальное динамическое добавление по ключу**:
\`\`\`js
...
\`\`\`
📌 **Разбор**: Синтаксис вычислимых свойств \`[key]: value\` позволяет передавать любое название ключа и значение динамически.

#### 📌 Ключевые выводы:
• Оборачивание объекта в скобки \`({ ... })\` необходимо для корректного возврата из стрелочной функции.
• Использование spread гарантирует неизменяемость (immutability) исходных объектов.`,
...
1.
**Анализ поведения неявного return**:
\`\`\`js
const arr = [1, 2, 3];
...
\`\`\`
📌 **Разбор**: Метод \`map()\` обязательн должен сформировать элемент на каждой позиции массива. Для чисел \`1\` и \`3\` ветка \`if\` не срабатывает, явный \`return\` отсутствует, поэтому функции по умолчанию возвращает \`undefined\`. В результате получаем \`[undefined, 2, undefined]\`.
...
1.
**Передача числовой функции компаратора**:
\`\`\`js
...
1.
**Сортировка по алфавиту без параметров**:
\`\`\`js
const fruits = ["banana", "apple", "cherry"];
...
1.
**Разделение массива и сортировка каждой группы**:
\`\`\`js
...
1.
**Анализ работы строковой сортировки**:
\`\`\`js
...
• Для математически корректной сортировки чисел **всегда** передавайте компаратор \`(a, b) => a - b\`.`,
  ...
  // ==========================================
  js51: `#### 🎯 Суть задачи
Запоминание и практическое применение основного синтаксиса метода \`Array.prototype.reduce()\`.

#### 🛠️ Пошаговые этапы решения:

1.
**Запись фундаментального шаблона**:
\`\`\`js
const result = arr.reduce((acc, item) => {
  return acc + item;
}, 0);
\`\`\`
📌 **Разбор**: На каждом шаге колбэк принимает накопленный аккумулятор \`acc\` и текущий элемент \`item\`. Значение \`0\` после запятой устанавливает стартовый аккумулятор.

#### 📌 Ключевые выводы:
• Всегда возвращайте обновленный \`acc\` из тела функции редюсера.`,
...
1.
**Обновление аккумулятора с Infinity**:
\`\`\`js
...
1.
**Накопление элементов через push и spread**:
\`\`\`js
const flatten = (arr) => {
  return arr.reduce((acc, el) => {
    acc.push(...el);
...
1.
**Использование nullish-оператора**:
\`\`\`js
const transactions = [
...
1.
**Вложенная группировка по ключам**:
\`\`\`js
...
1.
**Замыкание идентификатора таймера**:
\`\`\`js
...
1.
**Анализ порядка работы Event Loop**:
\`\`\`js
console.log('1');
...
1.
**Хронология работы двух интервалов**:
\`\`\`js
let counter = 0;
...
1.
**Разделение текста и дедупликация**:
\`\`\`js
...
1.
**Фильтрация по Set встречавшихся ID**:
\`\`\`js
...
1.
**Проверка объектов на равенство по ссылке**:
\`\`\`js
const a = { x: 1 };
const b = { x: 1 };
const set = new Set();
...
1.
**Проверка ключа с помощью Map.has()**:
\`\`\`js
...
1.
**Преобразование итераторов в массивы**:
\`\`\`js
...
1.
**Преобразование результата Map в объект**:
\`\`\`js
const groupBy = (items, key) => {
  const map = new Map();
  items.forEach((user) => {
...
1.
**Сохранение контекста и аргументов**:
\`\`\`js
function memoize(fn) {
  const cache = new Map();
...
1.
**Проверка состояния кэша и логирование**:
\`\`\`js
...
1.
**Создание промиса и отложенный resolve**:
\`\`\`js
const delay = (ms) => {
...
1.
**Оборачивание await в try/catch**:
\`\`\`js
...
1.
**Гонка ответа и таймаут-промиса**:
\`\`\`js
...
1.
**Цикл повторов с выдержкой задержки**:
\`\`\`js
...
1.
**Пул воркеров с общим индексом задач**:
\`\`\`js
...
1.
**Анализ очередности очереди макро/микрозадач**:
\`\`\`js
console.log("1");
...
1.
**Управление состоянием и подписчиками**:
\`\`\`js
class MyPromise {
...
1.
**Сброс предыдущего таймаута и отмена отклонением маркерным символом**:
\`\`\`js
...
1.
**Подсчет выполненных задач и сохранение порядка**:
\`\`\`js
...
1.
**Параллельные воркеры забирают задачи из очереди**:
\`\`\`js
...
1.
**Создание локальной переменной и возвращаемой функции**:
\`\`\`js
const createCounter = () => {
  let count = 0;
  return () => ++count;
...
1.
**Применение блочной области видимости let**:
\`\`\`js
for (let i = 0; i < 10; i++) {
  setTimeout(function () {
...
1.
**Перенос вычисления строки внутрь логирующей функции**:
\`\`\`js
...
1.
**Использование let в цикле по массиву**:
\`\`\`js
...
1.
**Условие остановки и рекурсивный шаг**:
\`\`\`js
const countDown = (n) => {
...
1.
**Умножение на основание на каждом шаге**:
\`\`\`js
...
1.
**Сравнение первого элемента с максимумом оставшейся части**:
\`\`\`js
const findMax = (arr) => {
...
1.
**Рекурсивная сплющивающая сборка**:
\`\`\`js
const flatten = (arr) => {
  const result = [];
  for (const el of arr) {
...
1.
**Рекурсивная репликация структуры**:
\`\`\`js
...
1.
**Рекурсивное суммирование с проверкой типов**:
\`\`\`js
const deepSum = (obj) => {
  let sum = 0;
...
1.
**Рекурсия по левой и правой ветвям**:
\`\`\`js
const treeSum = (node) => {
...
1.
**Суммирование отрицательных и положительных элементов**:
\`\`\`js
...
1.
**Накопление примитивов массивом**:
\`\`\`js
...
1.
**Влияние времени создания экземпляров**:
\`\`\`js
...
1.
**Мутация объекта по общей ссылке**:
\`\`\`js
...
1.
**Разница поиска в объекте и прототипе**:
\`\`\`js
...
1.
**Терминальное условие при вызове без аргумента**:
\`\`\`js
