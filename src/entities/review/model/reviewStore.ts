...
import { ReviewState, ReviewRating, ReviewItem, MasteryStats } from "../types";
import { calculateNextReview, isTaskDue, RATINGS, MAX_STAGE } from "./sm2Algorithm";
...
export const useReviewStore = create<ReviewState>((set, get) => ({
  ...
  submitReview: async (taskId: string | number, rating: ReviewRating = "medium") => {
    ...
    await saveReviewToDB(stringId, item as any);
    broadcastSyncEvent("TASK_REVIEWED", { taskId: stringId, review: item });
  },

  removeReview: async (taskId: string | number) => {
    const stringId = String(taskId);
    set((state) => {
      const updated = { ...state.reviews };
  ...
  getDueTasks: (customTaskList: any[] = []) => {
    ...
    return list
      ...
      .sort(
        (a: any, b: any) => (a.reviewData?.nextReviewAt || 0) - (b.reviewData?.nextReviewAt || 0)
      );
  },
  ...
  handleResetReviews: async (scope = "section", taskIds: Array<string | number> = []) => {
    if (scope === "all") {
