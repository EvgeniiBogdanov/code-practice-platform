### Что делает решение
Функция `createSecureEntity(id, secretToken, initialRole = "user")` использует метод `Object.defineProperty` для точечной настройки флагов дескриптора свойств (`Property Descriptors`), защищая объект от несанкционированной модификации, скрывая конфиденциальные токены от перечисления и настраивая контролируемые аксессоры (get/set).
