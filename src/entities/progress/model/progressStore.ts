...
export const useProgressStore = create<ProgressState>((set, get) => ({
  ...
  setTaskStatus: async (taskId: string | number, status: TaskCompletionStatus) => {
  ...
  toggleChecklistItem: async (key: string) => {
  ...
  handleCopyCode: (id: string, codeText: string) => {
    if (!codeText) return;
    try {
  ...
  handleFullReset: async (scope = "section", taskIds: Array<string | number> = []) => {
    if (scope === "all") {
