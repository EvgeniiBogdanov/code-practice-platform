...
export const JS_HINTS = {
  ...
  "js46": {
    "level1": {
      "title": "Идея и ментальная модель",
      "content": "Для сортировки объектов по числовому свойству: (a, b) => a[key] - b[key]. Для строковых свойств используйте a[key].localeCompare(b[key])."
    },
    "level2": {
      "title": "Граничные случаи и ловушки",
      "content": "• Не используйте a[key] - b[key] для строк, так как вычитание строк вернет NaN и сломает сортировку.\n• Обрабатывайте возможные undefined в свойствах."
    },
    "level3": {
      "title": "Псевдокод и сигнатура",
  ...
  "js49": {
    "level1": {
      "title": "Идея и ментальная модель",
      "content": "Многоуровневая сортировка: сравниваем элементы по первому полю; если они равны (разность === 0), переходим к сравнению по второму полю и так далее."
    },
    "level2": {
      "title": "Граничные случаи и ловушки",
      "content": "• Используйте оператор логического ИЛИ (||) для цепочки компараторов: cmp1(a, b) || cmp2(a, b) || cmp3(a, b).\n• Очень частая задача на собеседованиях при выводе табличных данных."
    },
    "level3": {
      "title": "Псевдокод и сигнатура",
  ...
  "js50": {
    "level1": {
      "title": "Идея и ментальная модель",
      "content": "Линейный поиск перебирает элементы один за другим от 0 до length - 1 за O(N) времени и O(1) памяти. При первом совпадении сразу возвращает индекс."
    },
    "level2": {
      "title": "Граничные случаи и ловушки",
      "content": "• Если элемент не найден, возвращайте -1.\n• Работает на неотсортированных данных."
    },
    "level3": {
      "title": "Псевдокод и сигнатура",
  ...
  "js51": {
    "level1": {
      "title": "Идея и ментальная модель",
      "content": "Бинарный поиск на отсортированном массиве: делим отрезок [left, right] пополам. Сравниваем середину mid с target и сужаем отрезок."
    },
    "level2": {
      "title": "Граничные случаи и ловушки",
      "content": "• Проверьте условие left <= right.\n• В JS вычисление mid = Math.floor((left + right) / 2) безопасно для обычных размеров массивов."
    },
    "level3": {
      "title": "Псевдокод и сигнатура",
