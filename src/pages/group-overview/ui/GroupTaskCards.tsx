...
import { Card, UiSkeleton, Tooltip } from "@/shared/ui";
import styles from "./GroupOverviewPage.module.css";

import { ReviewItem, useReviewStore } from "@/entities/review";
...
const GroupTaskCardItem = memo(
  ({
    ...
    const taskReview = reviews[stringId] || reviews[task.id];
    ...
    const tooltipTitle = isExcluded
      ? `${task.title} (Исключена из цикла повторений)`
      : getTaskTooltipTitle(task, s, taskReview);
        ...
        >
          <div>
              ...
              <div className={styles.galleryCardStatusRow}>
                ...
                {!isExcluded && (
                  isDue ? (
                    ...
                    <span className={styles.statusIconSolved}>
                      <Check size={13} />
                    </span>
                  ) : isUnsolved ? (
            ...
            <div className={styles.galleryCardTitleRow}>
              ...
              <span
                className={clsx(
                  styles.galleryCardTitleText,
                  isExcluded && styles.galleryCardTitleExcluded
                )}
              >
                {task.title}
              </span>
          ...
          {!isExcluded && (lastReviewedAt || nextReviewAt || isDue) && (
