...
import { formatLastSolved } from "../lib/formatLastSolved";
import {
...
export const useGroupOverview = (groupId: string): GroupOverviewState => {
  ...
  const { groupMeta, groupTasks, section } = useMemo(() => {
    ...
    const reactGroups = REACT_GROUPS_CONFIG as Record<string, any>;
    const jsGroups = JS_GROUP_CONFIG as Record<string, any>;

    if (reactGroups[groupId]) {
      sec = "react";
      const cfg = reactGroups[groupId];
      meta = {
        name: cfg.name || cfg.title,
