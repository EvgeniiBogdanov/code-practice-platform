...
/*
=== Разбор решения ===
Проблема: Один гигантский тип с множеством опциональных полей (`permissions?: string[]`, `department?: string`) приводит к ситуации, когда у администратора может случайно не оказаться `permissions`, а TS не сможет подсказать ошибку во время сборки.

Как надо (React + TS):
1. **Discriminated Union (Размеченное объединение)** — это объединение типов с общеизвестным полем-дискриминантом (`role`).
2. **Type Guard (`user is AdminUser`)** — функция-предикат, позволяющая языку безошибочно понимать, к какому подтипу относится объект в ветке `if`.
3. Никаких опасных приведений типов `as string[]` или рисков обратиться к `undefined`.
*/
