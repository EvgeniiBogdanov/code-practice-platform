...
export function getRobotMessage({
  ...
  if (isUnsolved) {
    ...
    if (daysElapsed >= 30) {
      const idx = getMessageIndex(
        [task?.id, "unsolved_month", statusUpdatedAt],
        UNSOLVED_MONTH_MESSAGES.length
      );
    ...
    if (daysElapsed >= 7) {
      const idx = getMessageIndex(
        [task?.id, "unsolved_week", statusUpdatedAt],
        UNSOLVED_WEEK_MESSAGES.length
      );
    ...
    if (daysElapsed >= 1) {
      const idx = getMessageIndex(
        [task?.id, "unsolved_next_day", statusUpdatedAt],
        UNSOLVED_NEXT_DAY_MESSAGES.length
      );
  ...
  if (taskReview && !canRate) {
    ...
    if (stage >= 4 || taskReview.intervalDays >= 21) {
      return {
        text: "Впечатляет! Материал отлично осел в памяти. В день проверки подготовлю шаблон для контрольного прогона. Следующий чекап:",
        highlight: nextDate,
      };
    }
    ...
    if (taskReview.rating === "hard") {
      const idx = getMessageIndex(
        [task?.id, taskReview.lastReviewedAt, "hard"],
        HARD_SCHEDULED_MESSAGES.length
      );
    ...
    if (taskReview.rating === "medium") {
      const idx = getMessageIndex(
        [task?.id, taskReview.lastReviewedAt, "medium"],
        MEDIUM_SCHEDULED_MESSAGES.length
      );
    ...
    if (taskReview.rating === "easy") {
      const idx = getMessageIndex(
        [task?.id, taskReview.lastReviewedAt, "easy"],
        EASY_SCHEDULED_MESSAGES.length
      );
    ...
    if (section === "react") {
      return {
        text: "Отлично! В день повторения подготовлю редактор, чтобы освежить хуки и логику компонентов. Напомню:",
        highlight: nextDate,
      };
    }
    ...
    if (section === "algorithms") {
      return {
        text: "Зафиксировал! В день повтора сброшу код до шаблона для тренировки алгоритмической памяти. Напомню:",
        highlight: nextDate,
      };
    }

    return {
      text: "Отлично! В день повторения подготовлю шаблон для закрепления материала на практике. Напомню:",
      highlight: nextDate,
    };
  }
  ...
  if (canRate && taskReview) {
    ...
    if (overdueDays >= 30) {
      if (prevRating === "hard") {
        const idx = getMessageIndex(
          [task?.id, taskReview.lastReviewedAt, "overdue_m_hard"],
          OVERDUE_MONTH_HARD_MESSAGES.length
        );
        return { text: OVERDUE_MONTH_HARD_MESSAGES[idx] };
      }
      if (prevRating === "easy") {
        const idx = getMessageIndex(
          [task?.id, taskReview.lastReviewedAt, "overdue_m_easy"],
          OVERDUE_MONTH_EASY_MESSAGES.length
        );
        return { text: OVERDUE_MONTH_EASY_MESSAGES[idx] };
      }
      const idx = getMessageIndex(
        [task?.id, taskReview.lastReviewedAt, "overdue_m_med"],
        OVERDUE_MONTH_MEDIUM_MESSAGES.length
      );
    ...
    if (overdueDays >= 8) {
      if (prevRating === "hard") {
        const idx = getMessageIndex(
          [task?.id, taskReview.lastReviewedAt, "overdue_w2_hard"],
          OVERDUE_WEEK2_HARD_MESSAGES.length
        );
        return { text: OVERDUE_WEEK2_HARD_MESSAGES[idx] };
      }
      if (prevRating === "easy") {
        const idx = getMessageIndex(
          [task?.id, taskReview.lastReviewedAt, "overdue_w2_easy"],
          OVERDUE_WEEK2_EASY_MESSAGES.length
        );
        return { text: OVERDUE_WEEK2_EASY_MESSAGES[idx] };
      }
      const idx = getMessageIndex(
        [task?.id, taskReview.lastReviewedAt, "overdue_w2_med"],
        OVERDUE_WEEK2_MEDIUM_MESSAGES.length
      );
    ...
    if (overdueDays >= 1) {
      if (prevRating === "hard") {
        const idx = getMessageIndex(
          [task?.id, taskReview.lastReviewedAt, "overdue_w1_hard"],
          OVERDUE_WEEK1_HARD_MESSAGES.length
        );
        return { text: OVERDUE_WEEK1_HARD_MESSAGES[idx] };
      }
      if (prevRating === "easy") {
        const idx = getMessageIndex(
          [task?.id, taskReview.lastReviewedAt, "overdue_w1_easy"],
          OVERDUE_WEEK1_EASY_MESSAGES.length
        );
        return { text: OVERDUE_WEEK1_EASY_MESSAGES[idx] };
      }
      const idx = getMessageIndex(
        [task?.id, taskReview.lastReviewedAt, "overdue_w1_med"],
        OVERDUE_WEEK1_MEDIUM_MESSAGES.length
      );
    ...
    if (stage >= 4) {
      const idx = getMessageIndex(
        [task?.id, taskReview.lastReviewedAt, "due_master"],
        DUE_MASTER_MESSAGES.length
      );
    ...
    if (prevRating === "hard") {
      const idx = getMessageIndex(
        [task?.id, taskReview.lastReviewedAt, "due_hard_prev"],
        DUE_AFTER_HARD_MESSAGES.length
      );
    ...
    if (section === "react") {
      const idx = getMessageIndex(
        [task?.id, taskReview.lastReviewedAt, "due_react"],
        DUE_REACT_MESSAGES.length
      );
    ...
    if (section === "algorithms") {
      const idx = getMessageIndex(
        [task?.id, taskReview.lastReviewedAt, "due_algo"],
        DUE_ALGO_MESSAGES.length
      );
    ...
    if (difficulty === "hard" || difficulty === "сложно") {
      const idx = getMessageIndex(
        [task?.id, taskReview.lastReviewedAt, "due_diff_hard"],
        DUE_HARD_DIFFICULTY_MESSAGES.length
      );
      return { text: DUE_HARD_DIFFICULTY_MESSAGES[idx] };
    }

    const idx = getMessageIndex(
      [task?.id, taskReview.lastReviewedAt, "due_general"],
      DUE_GENERAL_MESSAGES.length
    );
