...
export function useHoverSignatures(): HoverSignaturesState {
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLTextAreaElement>, code: string) => {
    ...
    hoverTimeoutRef.current = setTimeout(() => {
      ...
      if (lineIdx >= 0 && lineIdx < lines.length) {
        ...
        if (colIdx >= 0 && colIdx <= line.length) {
          ...
          const info = getHoverInfo(code, offset);
          if (info) {
            setHoverInfo(info);
            setPosition({
  ...
  const updateSignatureHelp = useCallback((code: string, cursorPos: number) => {
    const help = getSignatureHelp(code, cursorPos);
